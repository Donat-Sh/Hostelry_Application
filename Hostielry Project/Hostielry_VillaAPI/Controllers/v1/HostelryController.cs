...
namespace HostelryAPI.Controllers.v1
{
    [Route("api/v{version:apiVersion}/Hostelry")]
    [ApiController]
    [ApiVersion("1.0")]
    public class HostelryController : ControllerBase
    {
        protected APIResponse _response;
        private readonly IHostelryRepository _dbVilla;
        private readonly IMapper _mapper;

        public HostelryController(IHostelryRepository dbVilla, IMapper mapper)
        {
            _dbVilla = dbVilla;
            _mapper = mapper;
            _response = new();
        ...
        public async Task<ActionResult<APIResponse>> GetVillas([FromQuery(Name ="filterOccupancy")] int? occupancy, [FromQuery] string? search, int pageSize = 0, int pageNumber = 1)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(search))
                    villaList = villaList.Where(u => u.Name.Contains(search)).ToList();
